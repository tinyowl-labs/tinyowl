<script lang="ts">
	...
	import {
		COVER_WORKSPACE_HEIGHT,
		COVER_WORKSPACE_WIDTH,
		MAX_CROP_ZOOM,
		MIN_CROP_ZOOM,
		clamp,
		exportCroppedCover,
	...
	const maxX = $derived(
		maxCropOffsetRect(imageWidth, baseScale, cropZoom, COVER_WORKSPACE_WIDTH),
	);
	const maxY = $derived(
		maxCropOffsetRect(imageHeight, baseScale, cropZoom, COVER_WORKSPACE_HEIGHT),
	);
	...
	$effect(() => {
		...
		void loadImageFromUrl(imageUrl)
			...
			.catch((e) => {
				cropError =
					e instanceof Error ? e.message : "Unable to preview image.";
			});
	});
	...
	async function save() {
		...
		cropError = "";
		try {
			const file = await exportCroppedCover({
				imageUrl,
				cropZoom,
				cropOffsetX,
				cropOffsetY,
			...
			cropError = e instanceof Error ? e.message : "Could not save cover.";
		} finally {
			busy = false;
		}
	}
...
<Dialog.Root bind:open>
	<Dialog.Portal>
		...
		/>
		<Dialog.Content
			class="fixed top-[50%] left-[50%] z-50 w-[calc(100%-2rem)] max-w-lg translate-x-[-50%] translate-y-[-50%] rounded-lg border border-border bg-background p-4 shadow-lg outline-none"
		>
			<div class="mb-3 flex items-start justify-between gap-3">
				<div>
					<Dialog.Title class="text-sm font-medium text-foreground"
						>Position cover</Dialog.Title
					>
					<Dialog.Description class="mt-0.5 text-xs text-muted-foreground"
						>Drag to position. Scroll or use +/− to zoom. Saved as
						16:9 WebP.</Dialog.Description
					>
				</div>
				<Dialog.Close
			...
			<div class="flex flex-col items-center gap-3">
				<div
					class="relative cursor-grab touch-none select-none overflow-hidden rounded-lg border border-border bg-muted active:cursor-grabbing"
					style="width: {COVER_WORKSPACE_WIDTH}px; height: {COVER_WORKSPACE_HEIGHT}px;"
					role="application"
					aria-label="Cover crop area"
					onwheel={onWheel}
				...
				>
					...
					{/if}
					<div
						class="pointer-events-none absolute inset-0 rounded-lg ring-1 ring-inset ring-background/40"
					></div>
				</div>
