<script lang="ts">
    ...
    import type { SearchProject, SimilarMediaItem, SearchEntityHit } from "$lib/search/discoveryLoad";
    import {
        clearImageQuery,
        loadImageQuery,
        type ImageQueryHit,
...
<SpatialDiscovery
    ...
    examples={data.browse
        ...
        : []}
    autofocus={Boolean(data.browse) || !activeQuery}
    title={data.browse
        ? "echidna"
        : data.query
          ? `${data.query} — echidna`
          : "echidna"}
    resultsHeading={data.browse
        ? hasSession
            ? "My projects"
            : "Projects"
        : ""}
>
    ...
    {#snippet empty()}
        <div class="py-6 text-muted-foreground">
            ...
            {#if data.browse}
                <p class="text-sm text-foreground">No projects yet</p>
                <p class="mt-1 text-xs">
                    {#if hasSession}
                        Create a project to see it here.
                    {:else}
                        Sign in to see your projects, or search to explore.
                    {/if}
                </p>
            {:else if !activeQuery}
