...
const {syncSelectionOverlay}=load('overlay'),{collectKeysAtScreenPoint}=load('pick');
const C=window.Cesium,results=[],errors=[];let renderer='unknown';
for(const [count,vertices,ground] of [[1,16,false],[1,256,false],[1,2048,false],[100,64,false],[1,256,true],[1,20000,false],[1000,128,false]]){
...
const ds=new C.CustomDataSource('base');await v.dataSources.add(ds);const items=[];
...
const e=ds.entities.add({id:String(i),polygon:{hierarchy:new C.PolygonHierarchy(positions),material:C.Color.BLUE.withAlpha(0.5),outline:true,outlineColor:C.Color.BLUE,...(ground?{height:0,heightReference:C.HeightReference.CLAMP_TO_GROUND}:{perPositionHeight:true})}});items.push({entity:e,kind:i===0?'primary':'secondary'});
}
...
await until(p=>p[0]<20 && p[2]>30);await new Promise(r=>setTimeout(r,300));
const samples=[];
...
results.push({count,vertices,ground,samples,gpuPickMs:Number(gpuPickMs.toFixed(2)),candidateScanWithoutBoundsMs:Number(exactMs.toFixed(2)),warmPicks,unchangedSelectionEntitiesRetained:retained,hits:keys.length});v.destroy();div.remove();
}
